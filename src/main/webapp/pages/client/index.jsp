		...
		<div id="book">
			...
			<div style="text-align: center">
				...
				<c:if test="${not empty sessionScope.cart.items}">
					<span>购物车中有${sessionScope.cart.totalCount}件商品</span>
					<div>
						刚刚将<span style="color: red">${sessionScope.lastName}</span>加入到了购物车
					</div>
				</c:if>
			</div>
			<c:forEach items="${requestScope.page.data}" var="book">
				<div class="b_list">
					...
					<div class="book_info">
						...
						<div class="book_add">
							<button itemid="${book.id}" class="addToCart">加入购物车</button>
						</div>
					</div>
				</div>
			</c:forEach>
	...
	<script>
		$(function () {
			...
			$(".addToCart").click(function () {
				...
				//添加商品到购物车
				location.href = "http://localhost:8080/bookshop/cart?action=addItem&id=" + itemid;
			})
		})
	</script>
</body>
